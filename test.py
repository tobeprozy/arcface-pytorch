...
def get_featurs(model, test_list, batch_size=10):
    ...
    for i, img_path in enumerate(test_list):
        ...
        if images.shape[0] % batch_size == 0 or i == len(test_list) - 1:
            ...
            data = data.to(torch.device("cpu"))[1::]
            output = model(data)
            
            modelonnx = model.module
            output_path = "resnet_arcface_fp32_1b.onnx"
            torch.onnx.export(modelonnx, data, output_path)
...
if __name__ == '__main__':
    ...
    model.to(torch.device("cpu"))  # 将模型放置在CPU上
